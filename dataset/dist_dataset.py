...
class DistLineReadingDataset(IterableDataset):  # pylint: disable=W0223
    ...
    def __init__(self,
        ...
        files_extend = []
        if sampling_rate:
            sampling_times = [int(x) for x in sampling_rate.split(',')]
            for f, t in zip(files, sampling_times):
                files_extend.extend([f] * t)
        else:
            files_extend = files
        print(f'files_extend', files_extend)
        self.files = hlist_files(files_extend)
