...
class ImageTextJsonDataset(DistLineReadingDataset):
    def __init__(self, config, data_path, rank=0, world_size=1, shuffle=True, repeat=True, transform=None, sampling_rate=None):
        super().__init__(data_path, rank, world_size, shuffle, repeat, sampling_rate=sampling_rate)
        ...
        print("dataset.sep_token", self.sep_token, flush=True)
        ...
        self.add_sep_token = config['add_sep_token']  # whether to add sep token at the end of sentence
    ...
    def preprocess(self, text):
        ...
        if self.add_sep_token:
            tokens = [self.cls_token] + tokens[:self.max_tokens - 2] + [self.sep_token]
        else:
...
class RegionTextJsonDataset(ImageTextJsonDataset):
    def __init__(self, config, data_path, rank=0, world_size=1, shuffle=True, repeat=True, transform=None, box_transform=None, sampling_rate=None):
        super().__init__(config, data_path, rank=rank, world_size=world_size, shuffle=shuffle,
                         repeat=repeat, transform=transform, sampling_rate=sampling_rate)
            ...
            try:
                ...
                if rand() < 0.5:
                    if self.hflip_mode==0 or \
                        (self.hflip_mode==1 and not self.left_or_right_in_caption(ann) ):
                        do_hflip = True
