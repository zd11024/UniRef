...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, register_blk=-1, idx_to_group_img=None, image_atts=None):
        ...
        if idx_to_group_img is None:
            return torch.cat([x_cls.transpose(1, 2), x], dim=1)  # (B, L+1, C)
        else:
            x_bs = torch.gather(x, dim=0, index=idx_to_group_img.view(-1, 1, 1).expand(-1, x.shape[1], x.shape[2]))  # (B2, L, C)
            weights = image_atts[:, 1:].unsqueeze(2)  # B L 1
            x_bs_cls = torch.sum((weights * x_bs).transpose(1, 2), dim=-1, keepdim=True)   # (B2, C, 1)
            x_bs_cls = x_bs_cls / torch.sum(weights.transpose(1, 2), dim=-1, keepdim=True)  # avgpool  (B2, C, 1)

            return torch.cat([x_bs_cls.transpose(1, 2), x_bs], dim=1), \
                   torch.cat([x_cls.transpose(1, 2), x], dim=1)  # (B2, L+1, C), (B, L+1, C)
