...
def build_text_encoder(config, vision_width, load_text_params=False, use_mlm_loss=False, config_text=None):
    ...
    config_text.region_threshold = config['region_threshold']
    config_text.detach_predictor = config['detach_predictor']
    config_text.pred_layers = [int(x) for x in config['pred_layers'].split(',')]
...
class XVLMBase(nn.Module):
    ...
    def get_text_embeds(self, text_ids, text_atts, token_type_ids=None):
        encoder = self.text_encoder.bert if hasattr(self.text_encoder, 'bert') else self.text_encoder
        return encoder(text_ids, attention_mask=text_atts, token_type_ids=token_type_ids, return_dict=True, mode='text').last_hidden_state

    def get_cross_embeds(self, image_embeds, image_atts, text_ids=None, text_embeds=None, text_atts=None, sel_index=None):
    ...
    def get_mlm_loss(self, text_ids_masked, text_atts, image_embeds, image_atts, masked_pos, masked_ids, token_type_ids=None):
    ...
    def predict_bbox(self, image_embeds, text_embeds, text_atts, ret_region_logits=False):
        """
        Args:
        ...
        output = self.get_cross_embeds(image_embeds, torch.ones(image_embeds.shape[:2]).to(image_embeds.device),
                                           text_embeds=text_embeds, text_atts=text_atts)
        output_cls = output[0][:, 0, :]
        output_coord = self.bbox_head(output_cls).sigmoid()
        
        if ret_region_logits:
            region_logits = output[-1]
            return output_coord, region_logits
        else:
            return output_coord
    
    def predict_bbox_partial(self, image_embeds, text_embeds, text_atts, ret_region_logits=False, sel_index=None):
        """
        Args:
            image_embeds: encoding full images
        ...
        output = self.get_cross_embeds(image_embeds, torch.ones(image_embeds.shape[:2]).to(image_embeds.device),
                                           text_embeds=text_embeds, text_atts=text_atts, sel_index=sel_index)
        # output_cls = output[0][:, 0, :]
        # output_cls = torch.index_select(output[0], dim=1, index=sel_index)  # (bs, n_hid)
        batch_size = image_embeds.size(0)
        output_cls = torch.gather(output[0], dim=1, index=sel_index.view(batch_size, 1, 1).repeat(1,1,output[0].size(-1))).squeeze(1)
        output_coord = self.bbox_head(output_cls).sigmoid()
